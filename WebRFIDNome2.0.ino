...
void handleIdentifiers() {  // Configura a rota para ver cadastros "/identificadores"
  ...
  String message = "<html>\
  <head>\
    <title>Numeros de Identificacao</title>\
    <style>\
      body { background-color: #cccccc; font-family: Arial, Helvetica, Sans-Serif; Color: black; }\
    </style>\
  </head>\
...
void removerIdentificador(const String &identifier) {  // Remove um identificador da memoria flash
  String identificadorR = identifier;
  String nomeR = "";
  String identifR = "";

  // Procura pelo identificador e ajusta identificadorR se encontrado
  for (int l = 0; l < identificationNumbers.size(); l++) {
    int pos3 = identificationNumbers[l].indexOf(',');
    if (pos3 != -1) {
      nomeR = identificationNumbers[l].substring(0, pos3);
      nomeR.trim();
      identifR = identificationNumbers[l].substring(pos3 + 1);
      identifR.trim();
    }
    if (identifier == identifR) {
      identificadorR = nomeR + "," + identifR;
      break;  // Identificador encontrado, sair do loop
    }
  }

  bool found = false;
  // Procura pelo identificador no vetor
  for (int i = 0; i < identificationNumbers.size(); i++) {
    if (identificationNumbers[i] == identificadorR) {
      identificationNumbers.erase(identificationNumbers.begin() + i);  // Remove o identificador do vetor
      found = true;
      break;
    }
  }
...
bool procurarID(String &identificador) {  // Funcao que procura um identificador no armazenamento
  ...
  for (int h = 0; h < identificationNumbers.size(); h++) {
    ...
    if (pos1 != -1) {                                  // Separa a identificacao em nome e numero de id
      ...
      nomeP = identificationNumbers[h].substring(0, pos1);
      nomeP.trim();
      // A segunda parte é da posição após a vírgula até o final
      identif = identificationNumbers[h].substring(pos1 + 1);
      identif.trim();
    }
    if (identificador == identif) {  // Retorna true se achar o numero de idenficacao
      return true;
    } else if (identificador == identificationNumbers[h]) {
      return true;
    }
  }
  return false;
...
void cadastro() {
  ...
  if (server.hasArg("nome")) {  // Identifica se teve alguma modificao no campo entrar com nome
    ...
    if (server.hasArg("identificacao")) {  // Identifica se teve alguma modificao no campo entrar com identificacao
      String identifC = server.arg("identificacao");
      identifC.trim();
      String identification = name + "," + identifC;             // Concatena as duas string, nome e id para uma idenficacao apenas
      identification.trim();                                    // Remove espaços em branco extras
      if (!identification.isEmpty() && !procurarID(identifC)) {  // Verificar se o Id ja existe
...
void remover() {
  ...
  if (server.hasArg("removedor")) {  // Identifica se teve alguma modificao no campo remover com identificacao
